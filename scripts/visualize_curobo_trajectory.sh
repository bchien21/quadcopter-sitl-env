...
if [[ $# -gt 3 ]]; then
    echo "Usage: $0 [dataset.h5] [first_trajectory_index] [number_of_trajectories]"
    exit 1
fi

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
workspace="$(dirname "$script_dir")/drone_ws"
dataset="${1:-/workspace/curobo_planning/datasets/warehouse_trajectories.h5}"
...
ros2 launch x500_moveit_config static_octomap_demo.launch.py \
    start_x:=0.0 start_y:=0.0 start_z:=3.0 start_yaw:=0.0 &
launch_pid=$!
